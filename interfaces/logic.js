...
function addPrimSilent(kind, pts){ const P={id:uid++,kind,pts:pts.map(p=>({x:p.x,y:p.y}))};
  const g=svg('g',{}); const hit=svg('path',{class:'prim-hit'}),body=svg('path',{class:'prim'}); g.append(hit,body); gP.appendChild(g);
...
function build(P){ const g=svg('g',{}); const hit=svg('path',{class:'prim-hit'}),body=svg('path',{class:'prim'}); g.append(hit,body); gP.appendChild(g); P.el={g,hit,body}; prims.push(P); wirePrim(P); draw(P); select(P); emit(); }
function pathOf(P){
...
function draw(P){ const e=P.el,d=pathOf(P); e.body.setAttribute('d',d); e.hit.setAttribute('d',d); e.g.classList.toggle('sel',selected===P); redraw(); drawCircuit(); }
...
function redraw(){
  ...
  if(selected && selected.pts && selected.kind!=='bubble'){ selected.pts.forEach((pp,idx)=>{ const c=svg('circle',{class:'edit',r:6,cx:pp.x,cy:pp.y});
  ...
  joinPts.forEach(J=>{ const linked=links.some(l=>l.a===J.id||l.b===J.id);
    const c=svg('circle',{class:'join'+(attachFrom===J.id?' hot':'')+(linked?' linked':''),r:7,cx:J.x,cy:J.y});

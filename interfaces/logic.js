...
function addPrimSilent(kind, pts){ const P={id:uid++,kind,pts:pts.map(p=>({x:p.x,y:p.y}))};
  const g=svg('g',{}); const hit=svg('path',{stroke:'transparent','stroke-width':16,fill:'none',style:'cursor:move'}),body=svg('path',{stroke:'#2b2b2b','stroke-width':3,fill:'none','stroke-linecap':'round',style:'cursor:move'}); g.append(hit,body); gP.appendChild(g);
...
function build(P){ const g=svg('g',{}); const hit=svg('path',{stroke:'transparent','stroke-width':16,fill:'none',style:'cursor:move'}),body=svg('path',{stroke:'#2b2b2b','stroke-width':3,fill:'none','stroke-linecap':'round',style:'cursor:move'}); g.append(hit,body); gP.appendChild(g); P.el={g,hit,body}; prims.push(P); wirePrim(P); draw(P); select(P); emit(); }
function pathOf(P){
...
function draw(P){ const e=P.el,d=pathOf(P); e.body.setAttribute('d',d); e.hit.setAttribute('d',d); e.body.setAttribute('stroke', selected===P ? 'var(--accent)' : '#2b2b2b'); redraw(); drawCircuit(); }

let wireLayer=svg('g',{}); vp.appendChild(wireLayer);
let ptLayer=svg('g',{}); vp.appendChild(ptLayer);
let pinLayer=svg('g',{}); vp.appendChild(pinLayer);
...
function drawCircuit(){
  ...
  terminals.forEach(T=>{ const g=svg('g',{}); g.setAttribute('transform','translate('+T.x+','+T.y+')');
    const c=svg('circle',{r:16,fill:'#fff',stroke:'#2b2b2b','stroke-width':2, style:'cursor:move'});
...
function redraw(){
  ptLayer.innerHTML='';
  if(selected && selected.pts && selected.kind!=='bubble'){ selected.pts.forEach((pp,idx)=>{ const c=svg('circle',{r:6,cx:pp.x,cy:pp.y,fill:'#eaf1f8',stroke:'var(--accent)','stroke-width':2,style:'cursor:grab'});
  ...
  if(selected && selected.pts && selected.kind!=='bubble' && !rotating){
    ...
    const HH=[['nw',x0,y0],['n',(x0+x1)/2,y0],['ne',x1,y0],['e',x1,(y0+y1)/2],['se',x1,y1],['s',(x0+x1)/2,y1],['sw',x0,y1],['w',x0,(y0+y1)/2]];
    HH.forEach(([pos,hx,hy])=>{ const h=svg('rect',{x:hx-4,y:hy-4,width:8,height:8,fill:'#fff',stroke:'var(--accent)','stroke-width':1.5,style:'cursor:pointer'});
  ...
  joinPts.forEach(J=>{ const linked=links.some(l=>l.a===J.id||l.b===J.id);
    const c=svg('circle',{r:7,cx:J.x,cy:J.y,fill:(attachFrom===J.id?'#f0b6ad':(linked?'#c0392b':'#fff')),stroke:'#c0392b','stroke-width':2,style:'cursor:pointer'});
    c.addEventListener('pointerdown',(ev)=>{ ev.stopPropagation(); onJoinClick(J); });
...
function onJoinClick(J){
  ...
  if(!attachFrom){ attachFrom=J.id; redraw(); return; }
  if(attachFrom===J.id){ attachFrom=null; redraw(); return; }
  const A=joinPts.find(j=>j.id===attachFrom), B=J;
...
const bPts=document.createElement('button'); bPts.textContent='Curve: 5 pts'; bPts.style.cssText='padding:7px 11px;border:1px solid var(--line);border-radius:7px;background:var(--paper);cursor:pointer;font-size:13px;'; bPts.onclick=()=>{ if(selected&&selected.kind==='curve'){ const cur=selected.pts.length; const target=cur===5?3:5; resampleCurve(selected,target); bPts.textContent='Curve: '+(target===5?'5':'3')+' pts'; draw(selected); emit(); } }; tb.appendChild(bPts);
...
const bDel=document.createElement('button');bDel.textContent='Delete';bDel.style.cssText='padding:7px 11px;border:1px solid var(--line);border-radius:7px;background:var(--paper);cursor:pointer;font-size:13px;';bDel.onclick=()=>{ if(!selected) return;
...
const bClr=document.createElement('button');bClr.textContent='Clear';bClr.style.cssText='padding:7px 11px;border:1px solid var(--line);border-radius:7px;background:var(--paper);cursor:pointer;font-size:13px;';bClr.onclick=()=>{prims.forEach(p=>p.el.g.remove());prims=[];joinPts=[];links=[];terminals=[];wires=[];inLetter=65;select(null);drawCircuit();emit();};tb.appendChild(bClr);

requestAnimationFrame(()=>{ fit(); setTool('select'); drawCircuit(); emit(); });
window.addEventListener('resize',()=>{fit();redraw();drawCircuit();}); applyView();
